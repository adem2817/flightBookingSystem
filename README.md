...
merchants sell products or services online. Flight ticket is one of these services.
...
it provides its payment service by calling bank endpoints. The bank responses are persisted to database.In [xxxPaymentServiceTest.java](src/test/java/com/xxx/challenge/service/xxxPaymentServiceTest.java)
...
In the simulation for some reason the bank response times take ~5 seconds. Due to this latency, a database connection problem is encountered after some time. (Running the [xxxPaymentServiceTest.java](src/test/java/com/xxx/challenge/service/IyzicoPaymentServiceTest.java)
...
* DatabaseConfiguration.java, BankService.java, PaymentServiceClients.java and xxxPaymentServiceTest.java classes must not be changed.
